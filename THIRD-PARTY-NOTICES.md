...
PadTools 本体は MIT ライセンス（ルートの `LICENSE` 参照）で配布されます。
配布される fat jar (`PadTools.jar`) には以下の第三者ライブラリが同梱されています。
...
- **PlantUML について**: PlantUML は同一機能を複数のライセンスの別アーティファクト
  ...
  しています。PadTools は fat jar 全体を MIT 互換に保つため **MIT 版 (`plantuml-mit`)**
  を採用しています。MIT 版は ditaa 連携など一部機能を含みませんが、PadTools が生成する
