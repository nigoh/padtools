...
Juml は **VS Code 風のタブ中心モデル**です。図を開くたびに右ペインへタブが追加され、
...
> **日本語フォントの既定 (2.x)**: フォントを未指定（空欄）にしておくと、Juml は
...
Juml 2.0 では、プロジェクトごとの設定が **SQLite**（`~/.juml/projects.db`）に自動保存されます。
...
*Juml 2.0 — MIT License — Copyright (c) 2015-2026 naou and contributors*
