...
```sh
java -Xmx8g -jar PadTools.jar -c -o carservice-class.svg \
...
```sh
java -Xmx8g -jar PadTools.jar -q "CarPropertyService.getProperty" \
...
```sh
java -jar PadTools.jar -M -o carservice-manifest.svg \
...
PadTools -G (Gradle 依存図) で可視化可能:

```sh
java -jar PadTools.jar -G -o car-deps.svg ~/AOSP/packages/services/Car
```
