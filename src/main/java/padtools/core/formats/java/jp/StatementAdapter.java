...
final class StatementAdapter {
    ...
    static void emitBody(BlockStmt body, List<JavaMethodInfo.Statement> out,
        ...
        List<padtools.core.formats.uml.JavaCommentScanner.Comment> direct =
    ...
    private static int flushComments(
        ...
        while (ci < direct.size() && direct.get(ci).start < beforeOffset) {
            String t = padtools.core.formats.uml.JavaCommentScanner.cleanText(direct.get(ci));
            if (t != null && !t.isEmpty()) {
    ...
    private static List<padtools.core.formats.uml.JavaCommentScanner.Comment> directComments(
            BlockStmt body, JpContext ctx) {
        List<padtools.core.formats.uml.JavaCommentScanner.Comment> out = new ArrayList<>();
        for (padtools.core.formats.uml.JavaCommentScanner.Comment c
    ...
    private static boolean captureFieldAssignmentInline(AssignExpr ae, JpContext ctx,
        ...
        for (padtools.core.formats.uml.JavaFieldInfo f : owner.getFields()) {
